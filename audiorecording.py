...
class microphone():
	def moving_average(self, data, sample_size):
		if (len(data) > 200):
			data = data[100: ]
		sum = 0
		sampled = 0
	...
	def shellCall(self, shellCmd, stdin='', stderr=False):
	...
	def record(self):
		...
		while True:
			...
			movavg = self.moving_average(datalist, 20)
			if (movavg > .5 or movavg < -.5):
				movavglist.append(1.0)
			else:
				movavglist.append(0.0)
			reco = self.moving_average(movavglist, 10)
			if (reco > 0.1):
				print "." * int(round(reco*10)**2)
				if (send == 0):
	...
	def wavtoflac(self, WAV_RECORDING):
		FLAC_CONV = 'flac -f '
		# Shitty transcoder stuff to convert wav to flac
		FLAC_PATH, _ = self.shellCall(['/usr/bin/which', 'flac'], stderr=True)
		...
		_, se = self.shellCall(flac_cmd, stderr=True)
